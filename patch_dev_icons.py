...
# Replace lucide imports and icons with raw SVGs
content = content.replace('import { Github, Linkedin } from "lucide-react";', '')
...
content = content.replace('<Github className="w-5 h-5" />', github_svg)
content = content.replace('<Linkedin className="w-5 h-5" />', linkedin_svg)
