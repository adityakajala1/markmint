...
export function ScopeCalculator() {
  ...
  const copyResults = () => {
    navigator.clipboard.writeText(`MarkMint GPA Estimate: ${gpa} | Target End Sem: ${requiredEndSem}`);
    setCopied(true);
    ...
    <div className="w-full max-w-2xl mx-auto flex flex-col items-center">
      ...
      <div className="flex bg-card border border-border p-1 rounded-md shadow-sm max-w-md mx-auto w-full mb-8">
        <button 
          ...
          className={`flex-1 py-2 px-4 rounded-md text-sm font-semibold transition-all ${activeTab === "endsem" ? "bg-accent text-accent-foreground shadow-md" : "text-foreground/60 hover:text-accent"}`}
        >
          End Semester
        </button>
        <button 
          onClick={() => setActiveTab("gpa")}
          className={`flex-1 py-2 px-4 rounded-md text-sm font-semibold transition-all ${activeTab === "gpa" ? "bg-accent text-accent-foreground shadow-md" : "text-foreground/60 hover:text-accent"}`}
        >
          Mint+ GPA
        </button>
      </div>
